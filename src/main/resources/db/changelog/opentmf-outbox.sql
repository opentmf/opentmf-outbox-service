...
-- §23 transactional outbox — the LIBRARY-OWNED table (opentmf-outbox-service; extracted
-- 2026-08-26 from the dnms-email-adapter reference). One per owning service's schema: the
...
comment on table outbox is '§23 transactional outbox (library-owned, opentmf-outbox-service): effects frozen at commit, relayed at-least-once in id order. Pending = relayed_on is null; parked = pending and attempts >= max-attempts; relayed rows pruned after the configured retention (default 7 days), parked rows never pruned automatically.';
...
comment on column outbox.client_profile is 'OPTIONAL named opentmf-http-clients profile for HTTP delivery (2026-08-26 hub ruling: per-row selector; null = publisher resolution — longest-prefix base-url match, else plain POST). Ignored by the Kafka publisher.';
...
comment on column outbox.headers is 'Optional serialized header map (§14 conventions) frozen at write time; the relay stamps x-idempotency-key/x-event-type/x-producer on top.';
