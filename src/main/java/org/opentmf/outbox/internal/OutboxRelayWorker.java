...
class OutboxRelayWorker {
  ...
  public int relayBatch() {
    List<OutboxEvent> claimed =
        repository.claimBatch(
            OffsetDateTime.now(ZoneOffset.UTC), properties.getMaxAttempts(), Limit.of(properties.getBatchSize()));
  ...
  private void registerFailure(OutboxEvent event, RuntimeException ex) {
    ...
    event.setNextAttemptOn(OffsetDateTime.now(ZoneOffset.UTC).plus(backoff.delayFor(attempts)));
